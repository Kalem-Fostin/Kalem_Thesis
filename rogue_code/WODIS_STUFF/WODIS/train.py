...
train_data = DataLoader(ASV_train, 
                        batch_size=cfg.BATCH_SIZE, # How many samples per batch to use 
                        shuffle=True,  # reshuffle at every epoch
                        num_workers=4) # 4 processes for loading the data

val_data = DataLoader(ASV_val, batch_size=cfg.BATCH_SIZE, shuffle=True, num_workers=4)  # Read validation data


wodis = WODIS_model(is_training=True, num_classes=3)
wodis = wodis.to(device)
criterion = nn.NLLLoss().to(device)
optimizer = optim.Adam(wodis.parameters(), lr=1e-4) # apply optimizer to optimize the model -- increasing speed and accuracy

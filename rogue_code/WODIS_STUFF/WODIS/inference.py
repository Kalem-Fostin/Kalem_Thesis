...
def main():
    ...
    '''
    # load model
    net = WODIS_model(is_training=False, num_classes=cfg.NUM_CLASSES).to(device) # lets get our cnn model
    ...
    # read label color from the class dict i.e. rgb from the class_dict.csv
    for i in range(num_class):
        tmp = pd_label_color.iloc[i]
        color = [tmp['r'], tmp['g'], tmp['b']]
        colormap.append(color) # create am rgb colour map
